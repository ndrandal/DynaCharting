 ...
 *
 * COMPOSED view — "correlation matrix" heatmap (ENC-537 / T4.5), using the
 * ViewTexture escape hatch (PR #22 / ENC-532), now ANIMATED (ENC-568).
 *
 * ── Animated texture track (ENC-568) ──────────────────────────────────────
 * The manifest still applies a STATIC base colormap texture on load (below), but
 * records.json now carries a `textures` TextureFrame[] track: 11 frames of a
 * ROLLING-WINDOW Pearson correlation (a 16-sample window slid across the tape),
 * each rasterized to an RGBA8 colormap. The replay engine swaps the bound texture
 * (textureId 700) via setTexturePixels at each frame's `t` (1 s apart, looping),
 * so the heatmap EVOLVES over time — the reference proof that the texture track
 * animates a texturedQuad. (Frames generated by gen-corr-frames alongside the
 * static build; re-run when the datasets change.)
 *
