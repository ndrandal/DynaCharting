 ...
 *                    not ambiguous.
 *
 * AND IT IS NOT A PROXY FOR THE ADAPTER. Measured 2026-09-22 on `candles-aapl`:
 * 59.8 Hz on the NVIDIA adapter and 60.2 Hz on SwiftShader (`--force-software`),
 * with 1.5ms and 1.1ms of renderer CPU respectively. rAF is clamped to the
 * display/compositor cadence and this scene does not saturate either device, so
 * the frame rate says nothing whatever about which one drew it. That is the whole
 * reason the adapter block above has to be a separate, enforced observation.
 *
