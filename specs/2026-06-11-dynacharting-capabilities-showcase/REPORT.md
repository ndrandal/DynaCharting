...
Every view now carries a **logical-chart chrome** overlay composited over the live canvas — driven entirely by the view's `chrome` metadata (`apps/showcase/views/*/view.json`), data-only per view: **axes** (gridlines + tick labels), **legends**, and **colorbars** (with the diverging/viridis ramps and their dB / Pearson-R scales). A small **FPS HUD** (toggle `F`, on by default) is pinned to each view, reading the EngineHost's own frame metrics inside the existing rAF loop — ~~it reads **~60 fps · 16.7 ms** across the captures, the proof the renderer holds frame budget while the chrome composites over it.~~ **STRUCK 2026-09-22 (ENC-1266) — no instrument ever produced that number, and none of it is re-checkable.** The capture path writes no frame-rate figure at all: the 47 committed JSON artifacts under `apps/showcase/stills/` (`render-tally.json`, 22 `*.png.capture.json`, 22 `*.png.probe.json`, `capture-manifest.json`, `contact-sheet.png.capture.json`) record coverage, chroma, distinct colours, adapter, replay progress and raster sha256 — and **no `fps` key in any of them** (`grep -lci fps apps/showcase/stills/*.json | wc -l` → `0`; repo-wide `git grep -lniE '"fps"' -- '*.json'` → nothing). The figure existed **only as pixels burned into the 2026-06 region screenshots**, and it did not read `~60 fps · 16.7 ms` there either — `git show 537c995:apps/showcase/stills/price-line-area.png` has **`1 fps · 1000.7 ms`** in its top-left corner, read back out of git and verified here. The ENC-1263 audit read all 22 of those badges and found the sentence false of roughly nine of them (`specs/2026-09-19-chart-quality-bar/PERF-CLAIMS.md` **C2**, in the workspace repo); that tally is **cited, not restated** — it is a measurement of pixels ENC-1288 has since overwritten, it cannot be re-taken, and it does not reconcile with itself (`12 at 60, 1 at 52, 2 at 2, 7 at 1` sums to 22 while C2's own list names ten views below 60). Two tickets have since removed even that record. **ENC-1265** found the `ms` half was `1000 / fps` — `dc::Stats::frameMs` was assigned nowhere in `core/`, so the badge printed one measurement twice under two unit labels, and `1000.7 = 1000/0.9993` is rounding, not information. **ENC-1288** (`cb92615`) then recaptured all 22 stills **canvas-only** through `canvas.toDataURL('image/png')`; the HUD is a DOM overlay (`src/chrome/FpsHud.tsx`, mounted by `ChromeOverlay.tsx:273`), so **no committed still contains a badge to read**. **No number replaces it here**, because there is no instrument behind one: what this engine does and does not measure is `LIMITATIONS.md` **DC-L-1265**, and that the showcase records nothing at all is **DC-L-1266**. `apps/showcase/tools/snap-stills.mjs` growing an adapter/fps record is ENC-1267.
