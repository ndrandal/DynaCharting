...
// derived from the current buffer size (vtxBytes / strideOf(pos2_clip=8B)) so a
// STREAMING line2d buffer draws the newly-appended segments instead of being
// frozen at the first frame's vertexCount.
void DawnLine2dBackend::buildGeoBuffers(GpuDevice& device, const Scene& scene,
                                        CpuBufferStore& gpu,
