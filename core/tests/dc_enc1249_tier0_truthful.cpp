...
//
//   D. DOJI (ENC-1251) — three candle6 records taken VERBATIM off the live
//      dataplane, one of them an open == close doji, mapped through the real
//      dc::LinearScale:
//        D1  the mark's full extent at cx is still low..high;
//        D2  the body is DRAWN — a record carrying an open and a close must
//            put ink at that level, which a degenerate body quad did not;
//        D3  the body sits AT the open/close level, not merely somewhere;
//        D4  a bar that already has height keeps exactly the height it had,
//            and D4b a floored doji body stays thin.
//
...
// ===========================================================================
// CASE D — THE DOJI, from records taken off the live wire (ENC-1251)
// ===========================================================================
//
...
//
// D2 is the assertion that FAILED before ENC-1251: a record carrying an open and
...
void caseDoji(dc::DawnSceneRenderer& renderer, const Mode& mode) {
  ...
  std::printf("\n-- D. DOJI: open == close, from the live wire (ENC-1251) --\n");
  ...
  for (int i = 0; i < kDojiN; ++i) {
    ...
    // D1 — the mark's full extent at cx is still exactly low..high. The
    ...
    check(extentOk,
          std::string("D1 [") + r.label + "] full extent at cx is low..high",
          fmt("got [%d..%d] want [%.1f..%.1f]", wick.top, wick.bottom, rowHigh,
              rowLow));

    // D2 — THE ENC-1251 ASSERTION. The body is DRAWN. A record that carries an
    ...
    check(body.count > 0,
          std::string("D2 [") + r.label + "] the body is drawn at all",
          fmt("x=%d lit=%d px%s", bodyX, body.count,
              r.doji ? "  (open == close: a DOJI)" : ""));

    // D3 — and it is drawn in the RIGHT PLACE: centred on the open/close level.
    ...
    check(body.count > 0 && std::fabs(gotMid - wantMid) <= kPlaceTolPx,
          std::string("D3 [") + r.label + "] the body sits at the open/close level",
          fmt("mid=%.1f want=%.1f (+-%.0f px)", gotMid, wantMid, kPlaceTolPx));

    // D4 — the floor is a FLOOR, not a resize: a bar that already has height
    ...
    if (!r.doji) {
      ...
      check(spanOk,
            std::string("D4 [") + r.label + "] a bodied bar still spans open..close",
            fmt("got [%d..%d] want [%.1f..%.1f]", body.top, body.bottom,
                std::min(rowOpen, rowClose), std::max(rowOpen, rowClose)));
    } else {
      // D4b — a doji body must stay THIN. Flooring it must not turn a
      ...
      check(body.count > 0 && spanPx <= 4,
            std::string("D4b [") + r.label + "] the floored doji body stays thin",
