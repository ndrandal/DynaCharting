...
**Verified at** `ENC-1252 HEAD`, 2026-09-19 — re-run because ENC-1252 changed
`vitest.config.ts` (the `include` this entry's third command runs under). All four residuals
read in source and unmoved; `npx vitest run packages/dc-wasm/src/EngineHost.rejections.test.ts`
green (4 tests), and green as part of `pnpm test` — now **20 files, 221 tests** (was 18/185 at
`5ac198a`; ENC-1252 adds `packages/dc-wasm/src/chart/domain.test.ts` and, via the widened
`include`, `apps/showcase/src/chrome/deriveAxes.test.ts`).
